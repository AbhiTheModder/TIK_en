...
def rmdire(path):
    if o_path.exists(path):
        ...
        try:
            ...
            ywarn("Cannot delete directory, insufficient permissions")
        else:
            ysuc("Deletion successful!")
...
class upgrade:
    ...
    def __init__(self):
        if not os.path.exists(temp):
            os.makedirs(temp)
        cls()
        with Console().status(f"[blue]Checking for new version...[/]"):
            try:
                data = requests.get(self.update_json).json()
            except (Exception, BaseException):
                data = None
        if not data:
            input("Failed to connect to server, press any button to return")
            return
        else:
            if data.get("version", settings.version) != settings.version:
                print(f"\033[31m {banner.banner1} \033[0m")
                print(
                    f"\033[0;32;40mNew version found:\033[0m\033[0;36;40m{settings.version} --> {data.get('version')}\033[0m"
                )
                print(
                    f"\033[0;32;40mUpdate log:\n\033[0m\033[0;36;40m{data.get('log', '1.Fix Some Bugs')}\033[0m"
                )
                input(
                    "Note, the communication group and the builds in release are always the latest development environment versions, this feature is only used to detect recent relatively stable builds"
                )
                try:
                    link = data["link"][plat.system()][plat.machine()]
                except (Exception, BaseException):
                    input(
                        "No update found for your device, please go to https://github.com/ColdWindScholar/TIK to download the source code and update it yourself"
                    )
                    return
                if not link:
                    input(
                    ...
                    return
                if input("\033[0;33;40mUpdate?[1/0]\033[0m") == "1":
                    ...
                    try:
                        ...
                        input("Update file corrupted, cannot update")
                        return
                    ...
                    input(
                        "Update complete, press any button to start the new program..."
                    )
                    subprocess.Popen(
                        [
                            os.path.join(
                ...
                input(
                    "\033[0;32;40mYou are using the latest version! Press any button to return!\033[0m"
                )
                return
...
class setting:
    def settings1(self):
        actions = {
            "1": lambda: settings.change(
                ...
                if (
                    brcom := input(
                        f"  Adjust brotli compression level (integer 1-9, the higher the level, the higher the compression rate, the longer it takes):"
                    )
                ).isdigit()
                and 0 < int(brcom) < 10
                else "1",
            ...
            "2": lambda: settings.change(
                "diysize",
                "1"
                if input("  Pack Ext image size [1] Dynamic minimum [2] Original size:")
                == "2"
                else "",
            ),
            "3": lambda: settings.change(
                "pack_e2",
                "0"
                if input("  Packing scheme: [1] make_ext4fs [2] mke2fs+e2fsdroid:")
                == "1"
                else "1",
            ),
            "6": lambda: settings.change(
                ...
                "1"
                if input(
                    "  Img whether to pack as sparse image (compress volume) [1/0]\n  Please enter the serial number:"
                )
                == "1"
                else "0",
            ),
            "7": lambda: settings.change(
                "diyimgtype",
                "1"
                if input(f"  Pack image system [1] Same as unpack format [2] Optional:")
                == "2"
                else "",
            ),
            "8": lambda: settings.change(
                "erofs_old_kernel",
                "1"
                if input(f"  EROFS packing whether to support old kernel [1/0]") == "1"
                else "0",
            ),
        }
        cls()
        ...
        \033[33m  > Packing Settings \033[0m
           1> Brotli Compression Level \033[93m[{settings.brcom}]\033[0m\n
           ----[EXT4 Settings]------
           2> Size Handling \033[93m[{settings.diysize}]\033[0m
           3> Packing Method \033[93m[{settings.pack_e2}]\033[0m\n
           ----[EROFS Settings]-----
           4> Compression Method \033[93m[{settings.erofslim}]\033[0m\n
           ----[IMG Settings]-------
           5> UTC Timestamp \033[93m[{settings.utcstamp}]\033[0m
           6> Create Sparse \033[93m[{settings.pack_sparse}]\033[0m
        ...
        if op_pro == "0":
            ...
            if input("  Select erofs compression method [1] Yes [2] No:") == "1":
                erofslim = input(
                    "  Select erofs compression method: lz4/lz4hc/lzma and compression level [1-9] (the larger the number, the longer it takes, the smaller the volume) e.g., lz4hc,8:"
                )
                settings.change("erofslim", erofslim if erofslim else "lz4hc,8")
            else:
                settings.change("erofslim", "lz4hc,8")
        elif op_pro == "5":
            if input("  Set packing UTC timestamp [1] Auto [2] Custom:") == "2":
                utcstamp = input("  Please enter: ")
                settings.change(
    ...
    def settings2(self):
        ...
        actions = {
            "1": lambda: settings.change(
                "super_group",
                super_group
                if (super_group := input(f"  Please enter (no special characters):"))
            ...
            "2": lambda: settings.change(
                ...
                if (
                    metadatasize := input(
                        "  Set metadata maximum reserved size (default is 65536, at least 512):"
                    )
                )
                else "65536",
            ),
            "3": lambda: settings.change(
                ...
                if (
                    BLOCKSIZE := input(
                        f"  Partition pack sector/block size: {settings.BLOCKSIZE}\n  Please enter: "
                    )
                )
                else "4096",
            ),
            "4": lambda: settings.change(
                ...
                if (
                    SBLOCKSIZE := input(
                        f"  Partition pack sector/block size: {settings.SBLOCKSIZE}\n  Please enter: "
                    )
                )
                else "4096",
            ),
            ...
            "6": lambda: settings.change(
                "fullsuper",
                ""
                if input("  Whether to force create Super image? [1/0]") != "1"
                else "-F",
            ),
            "7": lambda: settings.change(
                "autoslotsuffixing",
                ""
                if input("  Whether to mark partitions that need Slot suffix? [1/0]")
                != "1"
                else "-x",
            ),
        }
        ...
        \033[33m  > Dynamic Partition Settings \033[0m
           ...
           ----[Partition Settings]------
           3> Default Sector/Block Size \033[93m[{settings.BLOCKSIZE}]\033[0m\n
           ----[Super Settings]-----
           ...
           6> Force Generate Full Img \033[93m[{settings.fullsuper}]\033[0m
    ...
    \033[33m  > Tool Settings \033[0m\n
       1> Custom Home Banner \033[93m[{settings.banner}]\033[0m\n
       2> Online Mode \033[93m[{settings.online}]\033[0m\n
       3> Contexts Patch \033[93m[{settings.context}]\033[0m\n
       4> Check for Updates \n
       0> Return to Previous Menu\n
       --------------------------
            """)
        ...
        if op_pro == "0":
            ...
            print(
                f"  Home banner: [1] TIK5 [2] Hammer and Sickle [3] TIK2 [4] Genshin Impact [5] DXY [6] None"
            )
            banner_i = input("  Please enter the serial number: ")
            if banner_i.isdigit():
                if 0 < int(banner_i) < 7:
    ...
    def settings4():
        ...
        print(
            f"\033[93mOpen Source Address:\033[0m \033[91mhttps://github.com/ColdWindScholar/TIK\033[0m"
        )
        print(f"\033[93mSoftware Version:\033[0m \033[44mAlpha Edition\033[0m")
        print(
            f"\033[93mOpen Source License:\033[0m \033[68mGNU General Public License v3.0 \033[0m"
        )
        print("\033[31m---------------------------------\033[0m")
        print(f"\033[93mSpecial Thanks:\033[0m")
        print("\033[94mAffggh")
        print("Yeliqin666")
        print("YukongA")
        print("\033[0m")
                    ...
                    if group_name != "info":
                        for con in group_data["controls"]:
                            ...
                            if con["type"] == "text":
                                ...
                                ysuc("Please drag the file or enter the path below")
                                ...
                                for option in options:
                                    ...
                                    gavs[str(cs)] = value
                                print("---------------------------")
                                op_in = input("Please enter your choice:")
                                ...
                                if "text" in con:
                                    print(con["text"])
                                self.gavs[input_var_name] = input(
                                ...
                                print("Unsupported parsing:%s" % con["type"])
...
class Tool:
    ...
    def main(self):
        ...
        if settings.online == "true":
            try:
                ...
                print(
                    f"\033[36m “Open source is a forward journey without asking where it's from”\033[0m\n"
                )
            else:
            ...
            print(
                f"\033[36m “Open source is a forward journey without asking where it's from”"
            )
        print(" >\033[33m Project List \033[0m\n")
        print("\033[31m   [00]  Delete Project\033[0m\n\n", "  [0]  New Project\n")
        ...
        print("  --------------------------------------")
        print(
            "\033[33m  [55] Unpack  [66] Exit  [77] Settings  [88] Download ROM\033[0m\n"
        )
        op_pro = input("   Please enter the serial number：")
        if op_pro == "55":
            self.unpackrom()
        elif op_pro == "88":
            ...
            for op in op_pro:
                if op in projects.keys():
                    if input(f"   Confirm delete {projects[op]}？[1/0]") == "1":
                        rmdire(o_path.join(LOCALDIR, projects[op]))
                    else:
                        ywarn("Cancel deletion")
        elif op_pro == "0":
            projec = input("Please enter the project name (no Chinese):")
            if projec:
            ...
            cls()
            ysuc("\nThank you for using TI-KITCHEN5, goodbye!")
            sys.exit(0)
        elif op_pro == "77":
            setting()
        elif op_pro.isdigit():
    ...
    def project(self):
        ...
        print(
            f"  Project：{self.pro}\033[91m(Incomplete)\033[0m\n"
        ...
        print("\033[33m    0> Return to Home      2> Unpack Menu\033[0m\n")
        print("\033[36m    3> Pack Menu      4> Plugin Menu\033[0m\n")
        print("\033[32m    5> One-Click Packaging     6> Custom Features\033[0m\n")
    ...
    def merge_images(self):
        ...
        # Check if there are any files matching the pattern "super.img*[0-9]"
        ...
        # Check if there are any files with a single digit at the end
        single_digit_files = [f for f in files if len(f.split(".")[-1]) == 1]
        double_digit_files = [f for f in files if len(f.split(".")[-1]) == 2]

        if single_digit_files:
            output_dir = os.path.join("TI_out")
            os.makedirs(output_dir, exist_ok=True)

            output_file = os.path.join(output_dir, "super_chunk.raw.img")
            command = (
                ["simg2img"] + single_digit_files + double_digit_files + [output_file]
            )

            print(".....Merging super_chunk.raw.img...")
    ...
    def custom_rom(self):
        cls()
        print(" \033[31m>Custom Menu \033[0m\n")
        print(f"  Project：{self.pro}\n")
        print("\033[33m    0> Return to Previous  1> Magisk Patch\033[0m\n")
        print("\033[33m    2> Remove avb   3> Remove data encryption\033[0m\n")
        print("\033[33m    4> Merge *sparsechunk*\033[0m\n")
        op_menu = input("    Please enter the number: ")
        if op_menu == "0":
            return
        elif op_menu == "1":
    ...
    def magisk_patch(self):
        ...
        if op_menu in boots.keys():
            mapk = input("    Please enter the path of Magisk.apk:")
    ...
    def hczip(self):
        cls()
        project = LOCALDIR + os.sep + self.pro
        print(" \033[31m>Package ROM \033[0m\n")
        print(f"  Project：{os.path.basename(project)}\n")
        print(
            "\033[33m    1> Direct Packaging     2> Card-Line Integration \n    3> Return\033[0m\n"
        )
        ...
        if chose == "1":
            ...
            utils.dbkxyt(
                ...
                input("Packaging card-line integration limited model code:"),
    ...
    def unpackrom(self):
        ...
        if dir_has(LOCALDIR, ".zip"):
            ...
            ywarn("   No ROM files!")
        print("--------------------------------------------------\n")
        zipd = input("Please enter the corresponding serial number：")
        if zipd.isdigit():
            if int(zipd) in zips.keys():
                projec = input("Please enter the project name (can be left blank)：")
                project = (
                    "TI_%s" % projec
                    if projec
                ...
                with Console().status("[yellow]Unpacking ROM...[/]"):
...
class zip_file:
    def __init__(self, file, dst_dir, local, path=None):
        ...
        if os.path.exists(relpath):
            ywarn(
                f"There are documents of the same name:{file}，Automatically renamed it{(relpath := path + utils.v_code() + file)}"
            )
        with zipfile.ZipFile(
            ...
            for file in get_all_file_paths("."):
                ...
                try:
                    ...
                    print("Write{}Error{}".format(file, e))
        if os.path.exists(relpath):
            print(f"Package is complete:{relpath}")
        os.chdir(local)
    ...
    if op_pro == "66":
        ...
        if int(op_pro) in mysubs.keys():
            ...
            if os.path.exists(plugin_path + os.sep + "main.sh"):
                ...
                ywarn(f"{mysubs[int(op_pro)]} is an environment plugin, cannot be run!")
...
class installmpk:
    def __init__(self, mpk):
        ...
        ----------------
        """)
        print("Plugin Name: " + self.mconf.get("module", "name"))
        print(
            "Version:%s\nAuthor：%s"
            % (
...
class unmpk:
    def __init__(self, plug, name, moduledir):
        ...
        if plug:
            ...
            ywarn("Please select the plug -in!")
            input("Any button continues")

    def ask(self):
        cls()
        print(f"\033[31m >delete{self.value2} \033[0m\n")
        if self.arr2:
            print("\033[36mWill uninstall the following plug -in at the same time")
            print("\n".join(self.arr2))
            print("\033[0m\n")
        self.unloop() if input("Are you sure you uninstalled? [1/0]") == "1" else ysuc(
            "Cancel"
        )
        input("Any button continues")
    ...
    def umpk(self, name=None) -> None:
        if name:
            ...
            ywarn(f"uninstall{name}fail!") if os.path.exists(
                self.moddir + os.sep + name
            ) else yecho(f"uninstall{name}success!")


def unpack_choo(project):
    cls()
    os.chdir(project)
    print(" \033[31m >Decompose \033[0m\n")
    filen = 0
    files = {}
    infos = {}
    ywarn(f"  Please place files in the root directory of {project}!\n")
    print(" [0]- Decompose all files\n")
    if dir_has(project, ".br"):
        print("\033[33m [Br] Files\033[0m\n")
    ...
    if dir_has(project, ".new.dat"):
        print("\033[33m [Dat] Files\033[0m\n")
        ...
        for dat10 in os.listdir(project):
            if dat10.endswith(".dat.1"):
                if os.path.isfile(os.path.abspath(dat10)):
                    filen += 1
                    print(f"   [{filen}]- {dat10} <Segmented DAT>\n")
                    files[filen] = dat10
                    infos[filen] = "dat.1"
    if dir_has(project, ".img"):
        print("\033[33m [Img] Files\033[0m\n")
    ...
    if dir_has(project, ".ozip"):
        print("\033[33m [Ozip] Files\033[0m\n")
    ...
    if dir_has(project, ".ofp"):
        print("\033[33m [Ofp] Files\033[0m\n")
    ...
    if dir_has(project, ".ops"):
        print("\033[33m [Ops] Files\033[0m\n")
    ...
    if dir_has(project, ".win"):
        print("\033[33m [Win] Files\033[0m\n")
        ...
        for win0000 in os.listdir(project):
            if win0000.endswith(".win000"):
                if os.path.isfile(os.path.abspath(win0000)):
                    filen += 1
                    print(f"   [{filen}]- {win0000} <Segmented WIN> \n")
                    files[filen] = win0000
                    infos[filen] = "win000"
    if dir_has(project, ".dtb"):
        print("\033[33m [Dtb] Files\033[0m\n")
        for dtb0 in os.listdir(project):
            if dtb0.endswith(".dtb"):
                if (
                    ...
                    files[filen] = dtb0
                    infos[filen] = "dtb"
    print("\n\033[33m  [00] Return  [77] Loop Unpack  \033[0m")
    print("  --------------------------------------")
    filed = input("  Please enter the corresponding serial number：")
    if filed == "0":
        for v in files.keys():
            unpack(files[v], infos[v], project)
    elif filed == "77":
        imgcheck = 0
        upacall = input("  Unpack all files? [1/0]")
        for v in files.keys():
            if upacall != "1":
        ...
        unpack(files[int(filed)], infos[int(filed)], project) if int(
            filed
        ) in files.keys() else ywarn("Input error!")
    else:
        ywarn("Input error!")
    input("Press any button to continue")
    unpack_choo(project)
    ...
    if project:
        ...
        if filed == "0":
            ...
            if settings.diyimgtype == "1":
                imgtype = input(
                    "Manually pack all partition formats as: [1] ext4 [2] erofs [3] f2fs:"
                )
                if imgtype == "1":
                    imgtype = "ext"
                elif imgtype == "2":
            ...
            for f in parts.keys():
                imgcheck = (
                    input(f"  Pack {parts[f]}? [1/0]  ")
                    if input("  Pack all images? [1/0]  ") != "1"
                    else "1"
                )
                if not imgcheck == "1":
                    continue
    ...
    for dtbo_files in os.listdir(dtbodir + os.sep + "dtbo_files"):
        if dtbo_files.startswith("dtbo."):
            ...
            if (
                ...
                ywarn(f"Failed to decompile {dtbo_files}!")
    ysuc("Completed!")
    time.sleep(1)
...
def makedtbo(sf, project):
    ...
    try:
        ...
        ywarn(f"Failed to generate {os.path.basename(sf).split('.')[0]}.img!")
    else:
        ysuc(f"{os.path.basename(sf).split('.')[0]}.img has been generated!")
...
def packsuper(project):
    ...
    cls()
    ywarn(f"Please place the partition images to be packed in {project}{os.sep}super!")
    supertype = input("Please enter the Super type: [1]A_only [2]AB [3]V-AB-->")
    if supertype == "3":
        supertype = "VAB"
    elif supertype == "2":
        supertype = "AB"
        ...
        supertype = "A_only"
    isreadonly = input("Set partitions as read-only? [1/0]")
    ifsparse = input("Pack as sparse image? [1/0]")
    if not os.listdir(project + os.sep + "super"):
        print(
            "It seems that you don't have any partitions to pack. Do you want to move the following partitions for packing?"
        )
        move_list = []
        for i in os.listdir(project + os.sep + "TI_out"):
            if os.path.isfile(os.path.join(project + os.sep + "TI_out", i)):
                if gettype(os.path.join(project + os.sep + "TI_out", i)) in [
                    ...
                    move_list.append(i)
        print("\n".join(move_list))
        if input("Are you sure you want to proceed? [Y/N]") in ["Y", "y", "1"]:
            for i in move_list:
                shutil.move(
    ...
    checkssize = input(
        f"Please set the Super.img size:[1]9126805504 [2]10200547328 [3]16106127360 [4]Tool Recommended: {tool_auto_size} [5]Custom"
    )
    if checkssize == "1":
        ...
        supersize = input("Please enter the size of the super partition (in bytes):")
    yecho("Packing to TI_out/super.img...")
    insuper(
...
def insuper(Imgdir, outputimg, ssize, stype, sparsev, isreadonly):
    ...
    ywarn("Failed to create super.img!") if call(f"lpmake {superpa}") != 0 else ysuc(
        "Successfully created super.img!"
    )


def packpayload(project):
    if ostype != "Linux":
        print(f"Unsupported OS: {ostype}, currently only supports: Linux(aarch64&x86)")
        input("Press any key to continue")
        return
    if os.path.exists(project + os.sep + "payload"):
        if input("Previous Payload packing residue found, clear it? [1/0]") == "1":
    ...
    ywarn(f"Please place all partition images in {project + os.sep}payload!")
    yecho(
        "This feature is time-consuming, CPU-intensive, and memory-intensive. It is not very meaningful if there is no official signature, please consider it before using."
    )
    if not os.listdir(project + os.sep + "payload"):
        print(
            "You don't seem to have any partitions to pack. Do you want to move the following partitions for packing?"
        )
        move_list = []
        for i in os.listdir(project + os.sep + "TI_out"):
            if os.path.isfile(os.path.join(project + os.sep + "TI_out", i)):
    ...
    tool_auto_size = versize(tool_auto_size)
    checkssize = input(
        f"Please set the size of the Super.img: [1]9126805504 [2]10200547328 [3]Tool Recommended: {tool_auto_size} [5]Custom"
    )
    if checkssize == "1":
        supersize = 9126805504
    elif checkssize == "2":
        ...
        supersize = input("Please enter the size of the super partition (in bytes): ")
    yecho(f"Packing to {project}/TI_out/payload...")
    inpayload(supersize, project)


def inpayload(supersize, project):
    yecho("Packing to: TI_out/payload, payload.bin & payload_properties.txt")
    partname = []
    super_list = []
    pimages = []
    ...
    if not os.path.exists(out):
        input("Error, file not written")
    else:
        LOGS("Successfully created payload!") if call(
            f"delta_generator --in_file={out} --properties_file={project + os.sep + 'config' + os.sep}payload_properties.txt"
        ) == 0 else LOGE("Failed to create payload!")
...
def unpack(file, info, project):
    ...
    if info == "sparse":
        ...
        ofpm = input("What is the processor of the ROM model? [1] Qualcomm [2] MTK\t")
        if ofpm == "1":
        ...
        yecho(f"Partitions contained in {os.path.basename(file)}:")
        with open(file, "rb") as pay:
            print(
                f"{(parts_ := [i.partition_name for i in utils.payload_reader(pay).partitions])}"
            )
        extp = input(
            "Please enter the name of the partition to be extracted (space separated)/all[all]  "
        )
        if extp == "all":
        ...
        for fd in [f for f in os.listdir(project) if re.search(r"\.win\d+", f)]:
            with open(project + os.path.basename(fd).rsplit(".", 1)[0], "ab") as ofd:
                for fd1 in sorted(
                    ...
                    print(
                        "Merging %s to %s"
                        % (fd1, os.path.basename(fd).rsplit(".", 1)[0])
                    )
...
def autounpack(project):
    ...
    for infile in os.listdir(project):
        ...
        if os.path.isdir(os.path.abspath(infile)):
            ...
            continue
        if ask_ != "1":
            if not input(f"Do you want to unpack {infile}? [1/0]") == "1":
                continue
