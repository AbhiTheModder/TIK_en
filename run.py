        ...
        if os.name == "nt":
            for r, d, f in os.walk(path):
                for i in d:
                    if i.endswith("."):
                        call(
                            ["mv", str(os.path.join(r, i)), str(os.path.join(r, i[:1]))]
                        )
                for i in f:
                    if i.endswith("."):
...
class Welcome:
    def __init__(self, step: int = None):
        if step is None:
            ...
            self.step = step
        self.steps = {0: self.welcome, 1: self.language, 2: self.done}
        self.change_page()
    ...
    def language(self):
        language_list = {
            index + 1: lan
            for index, lan in enumerate(dir(languages))
            if lan != "default" and not lan.startswith("_") and not lan.endswith("_")
        }
    ...
    def __init__(self):
        ...
        if not data:
            ...
            if data.get("version", settings.version) != settings.version:
                ...
                input(
                    "\033[0;32;40mYou are using the latest version! Any button returns！\033[0m"
                )
                return
...
class setting:
    def settings1(self):
        actions = {
            "1": lambda: settings.change(
                ...
                if (
                    brcom := input(
                        "  Adjust the brotli compression level (integers 1-9):"
                    )
                ).isdigit()
                and 0 < int(brcom) < 10
                else "1",
            ...
            "2": lambda: settings.change(
                "diysize",
                "1"
                if input("  Package Ext image size: [1]Auto [2]Origin:") == "2"
                else "",
            ),
            "3": lambda: settings.change(
                "pack_e2",
                "0"
                if input("  Packing Method: [1]make_ext4fs [2]mke2fs+e2fsdroid:") == "1"
                else "1",
            ),
            "6": lambda: settings.change(
                "pack_sparse", "1" if input("  Pack to Sparse:[1/0]:") == "1" else "0"
            ),
            "7": lambda: settings.change(
                "diyimgtype",
                "1"
                if input(f"  Packaging Image Format [1]Origin [2]Selectable:") == "2"
                else "",
            ),
        ...
        if op_pro == "0":
            ...
            if (
                input("  Set packaging UTC timestamp [1] automatic [2] customization:")
                == "2"
            ):
    ...
    def settings2(self):
        ...
        actions = {
            ...
            "2": lambda: settings.change(
                ...
                if (
                    metadatasize := input(
                        "  Set the maximum reserved size for metadata (default to 65536, at least 512):"
                    )
                )
                else "65536",
            ),
            ...
            "6": lambda: settings.change(
                "fullsuper",
                ""
                if input("  Do you want to force the creation of Super images?[1/0]")
                != "1"
                else "-F",
            ),
            "7": lambda: settings.change(
                "autoslotsuffixing",
                ""
                if input("  Mark partitions that require a Slot suffix?[1/0]") != "1"
                else "-x",
            ),
        }
        print(f"""
    ...
    \033[33m  > Tool settings \033[0m\n
       ...
       --------------------------
        ...
        if op_pro == "0":
            ...
            print(
                f"  Banner: [1]TIK5 [2]Sickle and axe head [3]TIK2 [4]YuanSheng [5]DXY [6]None"
            )
            ...
            language_list = {
                ...
                for index, lan in enumerate(dir(languages))
                if lan != "default"
                and not lan.startswith("_")
                and not lan.endswith("_")
            }
            print("Select Your Language:")
            for index, lan in language_list.items():
                print(f"{index}>{lan}")
            language = language_list.get(
                int(input("Select Your Language:")), languages.default
            )
            settings.change("language", language)
        elif op_pro == "5":
            upgrade()
    ...
    def settings4():
        ...
        print(
            f"\033[93mOpen Source Link:\033[0m \033[91mhttps://github.com/ColdWindScholar/TIK\033[0m"
        )
...
class Tool:
    ...
    def main(self):
        ...
        if settings.online == "true":
            try:
                ...
                print(
                    f"\033[36m “Open source is a journey without any questions”\033[0m\n"
                )
            else:
    ...
    def custom_rom(self):
        ...
        print(
            "\033[33m    2> Disable avb(fstab)   3> Disable data_encryption(fstab)\033[0m\n"
        )
    ...
    def hczip(self):
        ...
        print(
            "\033[33m    1> Pack Directly     2> Pack Hybrid Rom \n    3> Return\033[0m\n"
        )
...
def subbed(project):
    ...
    if op_pro == "66":
        ...
        unmpk(mysubs[int(chose)], names[int(chose)], binner + os.sep + "subs") if int(
            chose
        ) in mysubs.keys() else ywarn("Invalid Number")
    elif op_pro == "0":
        return
    elif op_pro.isdigit():
...
def packChoo(project):
    ...
    if project:
        ...
        if filed == "0":
            ...
            for f in parts.keys():
                imgcheck = (
                    input(f"  Whether Pack {parts[f]}?[1/0]	")
                    if input("  Whether Pack All？ [1/0]	") != "1"
                    else "1"
                )
                if not imgcheck == "1":
                    continue
    ...
    if os.path.isdir(infile + os.sep + "ramdisk"):
        ...
        call(
            [
                ...
                "ash",
                "-c",
                f"find | sed 1d | {cpio} -H newc -R 0:0 -o -F ../ramdisk-new.cpio",
            ]
        )
        os.chdir(infile)
            ...
            if (
                call(
                    [
                        "magiskboot",
                        "decompress",
                        f"{project}/{name}/ramdisk.cpio.comp",
                        f"{project}/{name}/ramdisk.cpio",
                    ]
                )
                != 0
            ):
...
def undtb(project, infile):
    ...
    for i in track(os.listdir(dtbdir + os.sep + "dtb_files")):
        if i.endswith(".dtb"):
            ...
            call(
                ["dtc", "-@", "-I", "dtb", "-O", "dts", str(dtb), "-o", str(dts)], out=1
            )
    open(
        project
        + os.sep
        ...
        if (
            call(
                [
                    ...
                    dtb_,
                    "-o",
                    f"{dtbdir}/new_dtb_files/{new_dtb_files}.dtb",
                ],
                out=1,
            )
            != 0
    ...
    for dts_files in os.listdir(dtbodir + os.sep + "dts_files"):
        ...
        call(
            [
                ...
                dtb_,
                "-o",
                f"{dtbodir}/new_dtbo_files/{new_dtbo_files}",
            ],
            out=1,
        )
...
def inpacker(name, project, form, ftype, json_=None):
    ...
    if ftype == "erofs":
        ...
        call(
            [
                "mkfs.erofs",
                *other_,
                f"-z{settings.erofslim}",
                "-T",
                f"{utc}",
                f"--mount-point=/{name}",
                f"--fs-config-file={fs_config}",
                f"--product-out={os.path.dirname(out_img)}",
    ...
    if form in ["dat", "br"]:
        ...
        try:
            ...
            ...
        utils.img2sdat(
            out_img, project + "/TI_out", int(json_.get("dat_ver", "4")), name
        )
        try:
            os.remove(out_img)
        except (Exception, BaseException):
...
def packsuper(project):
    ...
    cls()
    ywarn(
        f"Please place the partition images that need to be packaged on the {project}{os.sep}super！"
    )
...
def insuper(Imgdir, outputimg, ssize, stype, sparsev, isreadonly):
    ...
    superpa = [
        "lpmake",
        "--metadata-size",
        f"{settings.metadatasize}",
        "--super-name",
        f"{settings.supername}",
    ]
    if sparsev == "1":
            ...
            if image not in supera_parts and f"{image}_a" not in supera_parts:
                if stype in ["VAB", "AB"]:
                    if os.path.isfile(
                        ...
                        superpa.append(
                            f"{image}_a:{attr}:{img_sizea}:{settings.super_group}_a"
                        )
                        superpa.append("--image")
                        superpa.append(f"{image}_a={Imgdir}/{image}_a.img")
                        superpa.append("--partition")
                        superpa.append(
                            f"{image}_b:{attr}:{img_sizeb}:{settings.super_group}_b"
                        )
                        superpa.append("--image")
                        ...
                        superpa.append("--partition")
                        superpa.append(
                            f"{image}_a:{attr}:{img_size}:{settings.super_group}_a"
                        )
    ...
    ywarn("Create super.img Fail！") if call(superpa) != 0 else ysuc(
...
def packpayload(project):
    ...
    if os.path.exists(project + "/payload"):
        if (
            input("Discovered residual Payload from previous packaging, empty it?[1/0]")
            == "1"
        ):
    ...
    tool_auto_size = (
        sum(
            [
                ...
                for p in os.listdir(project + "/payload")
    ...
    tool_auto_size = versize(tool_auto_size)
    checkssize = input(
        f"Choose Super.img Size:[1]9126805504 [2]10200547328 [3]{tool_auto_size} [5]自定义"
    )
    if checkssize == "1":
        supersize = 9126805504
    elif checkssize == "2":
    ...
    if not os.path.exists(out):
        ...
        LOGS("Done!") if call(
            [
                "delta_generator",
                f"--in_file={out}",
                f"--properties_file={project}/config/payload_properties.txt",
            ]
        ) == 0 else LOGE("Fail！")
...
def unpack(file, info, project):
    ...
    if info == "sparse":
        ...
        extp = input(
            "Please Enter Partition names need to be decompressed (separated by spaces)/all"
        )
        if extp == "all":
        ...
        for fd in [f for f in os.listdir(project) if re.search(r"\.win\d+", f)]:
            with open(project + os.path.basename(fd).rsplit(".", 1)[0], "ab") as ofd:
                for fd1 in sorted(
                    ...
                    print(
                        "Merge %s to %s" % (fd1, os.path.basename(fd).rsplit(".", 1)[0])
                    )
